    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          Citizen.CreateThread(function()
            while true do
              Wait(0)
              if (IsNearBank()) then
                drawTxt('Press ~g~E~s~ to access your account.',0,1,0.5,0.8,0.6,255,255,255,255)
              elseif (IsNearATM()) then
                drawTxt('Press ~g~E~s~ to access the ATM.',0,1,0.5,0.8,0.6,255,255,255,255)
              end
            end
          end)
        end
        ...
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You have to get out of your vehicle!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You are not at a bank!");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You must be in a bank or at an ATM!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You're not close enough to this player!");
  end
end)
