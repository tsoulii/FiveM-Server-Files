...
offense = {"WANTED BY LSPD","WANTED FOR ASSAULT","WANTED FOR UNPAID FINES","WANTED FOR RUNNING FROM THE POLICE","WANTED FOR EVADING LAW","WANTED FOR HIT/RUN","WANTED FOR DUI","WANTED FOR QUESTIONING OF A MURDER","WANTED FOR ARMED ROBBERY"}
illegalItems = {"a knife.","a pistol.","a fake ID card.","an illegal item.","an empty bottle of beer.","bags with suspicious white powder.","an AK-47.","an armed rifle.","a rifle.","a shotgun.","an UZI.","a weapon.","a baggie of coke.","a baggie of pot.","a briefcase full of cocaine.","a trunk full of cocaine.","a dead body in the trunk."}
----------------------
			...
			playerVeh = GetVehiclePedIsIn(player, false)
				if GetVehicleClass(playerVeh) == 18 then
					...
					elseif mimicking then
						...
						if GetIsVehicleEngineRunning(targetVeh) == false and DoesEntityExist(targetVeh) == false then
...
--RegisterCommand("breath", function()
	...
	if DoesEntityExist(breathVeh) then
		...
		if DoesEntityExist(breathDriver) then
			...
			Wait(2000)
			if breathNum >= 8 then
			ShowNotification("~b~BAC~w~ Level: ~r~0.0" .. breathNum)
			else
			ShowNotification("~b~BAC~w~ Level: ~g~0.0" .. breathNum)
			end
		end
	end
end)
